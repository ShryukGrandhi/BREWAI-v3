...
Answer with specific data and details."""
                        ...
                        with st.spinner("Generating voice with Gemini 2.5 Flash Preview TTS..."):
                            try:
                                ...
                                # Generate audio from Captain's response
                                tts_request = {
                                    'contents': [{
                                        'parts': [{'text': answer}]
                                    }],
                                ...
                                if hasattr(tts_response, 'parts'):
                                    for part in tts_response.parts:
                                        if hasattr(part, 'inline_data'):
                                            ...
                                            st.success("🔊 Voice generated by Gemini 2.5 Flash TTS! Press play above.")
                                            audio_generated = True
                                            break
                                
                                if not audio_generated:
                                    st.info("🔊 Gemini TTS response (text mode):")
                                    st.text_area("Voice Script", answer, height=150, key=f"tts_{text[:10]}")
                                
                            except Exception as e:
                                st.warning(f"Gemini TTS error: {e}")
                                st.info("🔊 Voice simulation:")
                                st.text_area("Would be read aloud", answer, height=150, key=f"tts_sim_{text[:10]}")
